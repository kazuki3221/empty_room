...
class StayDuplicationValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    ...
    if record.id.present?
      not_own_periods = Resevation.where('id NOT IN (?) AND start_stay <= ? AND end_stay >= ?', record.id, new_end_date, new_start_date)
    else
      not_own_periods = Resevation.where('start_stay <= ? AND end_stay >= ?', new_end_date, new_start_date)
    end
